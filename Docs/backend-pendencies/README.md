...
| Painel admin — Materiais da aula (no artboard) / "Material" tab on `1h` | [admin/lesson-materials.md](admin/lesson-materials.md) | Three compounding **Blocking** gaps: no real upload endpoint (an admin can't obtain a valid `StorageKey` at all), no student-facing list endpoint, and no download route (`GetDownloadUrlAsync` is dead code). Nothing works end-to-end yet on either the admin or student side. |
