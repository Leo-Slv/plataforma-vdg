...
| Painel admin — Vídeos (`1zc`) | [admin/videos-panel.md](admin/videos-panel.md) | No `GET /api/videos` collection endpoint exists at all (every video query is lesson-scoped) — Blocking. Compounded by three domain-model mismatches: no YouTube-id field (generic `StorageKey`), no "unlinked video" state (a `Video` always requires a `LessonId`), no "Ativo/Não listado" visibility status (only a `Processing/Ready/Failed` pipeline enum). |
