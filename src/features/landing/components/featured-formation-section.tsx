...
function FeaturedFormationSection({ course }: FeaturedFormationSectionProps) {
	...
	return (
		<section className="border-t border-foreground/8 px-5 py-11 sm:px-11 sm:py-22">
			<div className="grid grid-cols-1 items-center gap-8 overflow-hidden rounded-2xl bg-surface-2 sm:grid-cols-2 sm:gap-16">
				<div className="relative aspect-4/3">
					<CoverImage src={course.thumbnailUrl} className="opacity-50" />
					<div className="absolute inset-0 bg-gradient-to-r from-surface-2 via-transparent to-transparent" />
				</div>
				<div className="px-5 pb-8 sm:px-0 sm:pr-14 sm:pb-0">
					...
					<h3 className="mt-5.5 font-heading text-[32px] leading-[1.15] font-extralight tracking-tight text-foreground">
						{course.title}
					</h3>
					<p className="mt-3 font-heading text-[11px] tracking-[0.1em] text-foreground/45 uppercase">
						{statsLine} · {resolveLivePriceLabel(course)}
					</p>
					<p className="mt-4 text-[14.5px] leading-[1.7] font-light text-foreground/55">
						{course.description}
					</p>
					<div className="mt-7 flex gap-3.5">
						<Link
							href={detailHref}
							className="rounded-full bg-foreground px-6 py-3.5 text-[13.5px] text-background"
						>
							{featuredFormationCopy.primaryCtaLabel}
						</Link>
						<Link
							href={detailHref}
							className="px-1 py-3.5 text-[13.5px] text-foreground/70"
						>
